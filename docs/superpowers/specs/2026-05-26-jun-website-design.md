...
- 設計系統：色彩 token（暖橙點綴）、字體、間距尺標、亮／暗雙模式切換
- 全站版型外殼：導覽列、頁尾、響應式
- 本機 Supabase（Docker）串接：資料庫連線、環境變數、migration 流程
...
風格：瑞士國際風（Swiss Modernism）。

- **色彩**：暖橙 `#EA580C` 為唯一點綴色。中性色為近黑、純白與灰階。亮／暗各一套 token，以 CSS 變數切換
- **字體**：介面與標題用 Inter，中文搭 Noto Sans TC；標籤、日期、數字用等寬字（JetBrains Mono）。文章內文之後可加襯線字
- **字級尺標**：12 / 14 / 16 / 20 / 28 / 40 / 56
- **版面語言**：12 欄格線感、髮絲線分隔、大量留白、低圓角、單一點綴色的紀律
- **共用元件**：導覽列、頁尾、按鈕（黑白主色，互動態用點綴色）、輸入框、篩選膠囊、翻唱卡片／列、亮暗切換鈕
- **無障礙**：文字對比達 WCAG AA；點綴色不作為唯一資訊指示；支援鍵盤操作與 prefers-reduced-motion
...
- **前置需求**：Node.js、Docker Desktop（本機 Supabase 需要）、Supabase CLI
- **本機 Supabase**：以 Supabase CLI 啟動本機完整堆疊（Postgres、Auth、Studio），完全離線
